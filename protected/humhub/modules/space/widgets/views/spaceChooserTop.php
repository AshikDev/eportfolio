...
use humhub\modules\space\widgets\SpaceChooserItemTop;
...
<li class="dropdown">
    ...
    <ul class="dropdown-menu" id="space-menu-dropdown">
        ...
        <li>
            <ul class="media-list notLoaded" id="space-menu-spaces">
                <?php foreach ($memberships as $membership) : ?>
                    ...
                    <?php
                    if ($membership->space->community == '_0_') {
                        echo SpaceChooserItemTop::widget([
                            'space' => $membership->space,
                            'updateCount' => $membership->countNewItems(),
                            'isMember' => true
                        ]);
                        foreach ($memberships as $m) {
                            if (strpos($m->space->community, '_' . $membership->space->id . '_') !== false) {
                                echo SpaceChooserItemTop::widget([
                                    'space' => $m->space,
                                    'updateCount' => $m->countNewItems(),
                                    'isMember' => true
                                ]);
                            }
                        }
                ...
                <?php foreach ($followSpaces as $followSpace) : ?>
                    <?php
                    if ($followSpace->community == '_0_') {
                        echo SpaceChooserItemTop::widget([
                            'space' => $followSpace,
                            'isFollowing' => true
                        ]);
                        foreach ($followSpaces as $f) {
                            if (strpos($f->community, '_' . $followSpace->id . '_') !== false) {
                                echo SpaceChooserItemTop::widget([
                                    'space' => $f,
                                    'isFollowing' => true
                                ]);
        ...
        <?php if (Yii::$app->user->isAdmin()) : ?>
            <li>
                <div class="dropdown-footer">
                    <a href="#" class="btn btn-info col-md-12" data-action-click="ui.modal.load" data-action-url="<?= Url::to(['/space/create/create_community']) ?>">
                        <?= Yii::t('SpaceModule.widgets_views_spaceChooser', 'Create new community') ?>
                    </a>
                </div>
            </li>
        <?php endif; ?>
