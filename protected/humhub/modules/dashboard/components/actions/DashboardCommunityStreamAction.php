 ...
 * @author luke
 */
class DashboardCommunityStreamAction extends ActivityStreamAction
{
...
//            ->select(["contentcontainer.id"])
//            ->from('space')
//            ->where('community_id=' . $this->user->contentcontainer_id);
//        $union .= " UNION " . Yii::$app->db->getQueryBuilder()->build($publicSpacesSql)[0];
...
//            ->leftJoin('space sm', 'sm.id=space_membership.space_id')
//            ->leftJoin('contentcontainer', 'contentcontainer.pk=sm.id AND contentcontainer.class = :spaceClass')
//            ->where('space_membership.user_id=' . $this->user->id . ' AND space_membership.show_at_dashboard = 1 AND sm.community_id='.$this->user->contentcontainer_id);
...
//
//        // User can see private and public of his own profile (also when not created by hisself)
//        $condition .= ' (content.visibility = 0) ';
//
//        $this->activeQuery->andWhere($condition, [
//            ':userId' => $this->user->id,
//            ':userModel' => User::class,
...
//        ]);
//        
//        $contentIds = \humhub\modules\space\models\Space::find()->where(['community_id' => $this->user->contentcontainer_id])->all();
//        $conditionCommunity[] = 'OR';
//        if (!empty($contentIds)) {
//            foreach ($contentIds as $contentId) {
//                $conditionCommunity[] = ['content.contentcontainer_id' => $contentId->contentcontainer_id];
//            }
//        }
//        
//        $this->activeQuery->andWhere($conditionCommunity);
//    }
